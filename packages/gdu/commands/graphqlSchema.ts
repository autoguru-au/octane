...
async function loadLocalSchema(
	...
	try {
		schemaString = readFileSync(
			resolve(config.dirpath, config.schema as string),
			'utf8',
		);
	} catch (error) {
		debug(error);
...
async function getRemoteSchema(
	_config: GraphQLProjectConfig,
	endpointConfig: EndpointExtension,
): Promise<GraphQLSchema> {
	return (
		await new UrlLoader().load(endpointConfig.url, {
...
async function updateSchemaIfHasNew(
	config: GraphQLProjectConfig,
	endpointConfig: EndpointExtension,
) {
...
async function downloadNewSchema(
	config: GraphQLProjectConfig,
	endpointConfig: EndpointExtension,
) {
...
async function getSchemaChecksum(
	_config: GraphQLProjectConfig,
	endpointConfig: EndpointExtension,
): Promise<string> {
